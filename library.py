...
def signed_up():
    if request.method == 'POST':
        ...
        elif User_Login.query.filter(User_Login.name==request.form['name']).first() or User_Login.query.filter(User_Login.number==int((request.form['number']))).first():
            flash("User already registered")
        elif User_Login.query.filter(User_Login.username==request.form['user']).first():
            flash("Please choose a different Username")
        else:
            newuser=User_Login(name=request.form["name"],username=request.form["user"],password=request.form["password"],number=int(request.form["number"])
...
def issued_books_user():
    if 'user' in session:
        user=User_Login.query.filter(User_Login.username==session['user']).first()
        return render_template("issued-books-user.html",books=get_books(user))
    return redirect(url_for('logged_in'))
